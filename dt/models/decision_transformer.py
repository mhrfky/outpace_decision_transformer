...
class DecisionTransformer(TrajectoryModel):
    ...
    def __init__(
        ...
        self.predict_state_mean = torch.nn.Linear(hidden_size, self.state_dim)
        self.predict_state_logvar = torch.nn.Linear(hidden_size, self.state_dim)
        self.predict_action = nn.Sequential(
            *([nn.Linear(hidden_size, self.act_dim)] + ([nn.Tanh()] if action_tanh else []))
        )
        self.predict_return = torch.nn.Linear(hidden_size, 1)
    def get_state_embeddings(self,states):
        return self.embed_state(states)
    def forward(self, states, actions, rewards, returns_to_go, timesteps, attention_mask=None):
        ...
        state_mean_preds = self.predict_state_mean(x[:, 2])
        state_logvar_preds = self.predict_state_logvar(x[:, 2])
        action_preds = self.predict_action(x[:, 1])

        return state_mean_preds, state_logvar_preds, action_preds, return_preds
    ...
    def get_action(self, states, actions, rewards, returns_to_go, timesteps, **kwargs):
        ...
        _, _, action_preds, _ = self.forward(
            states, actions, None, returns_to_go, timesteps, attention_mask=attention_mask, **kwargs)

        return action_preds[0, -1]

    def get_state(self, states, actions, rewards, returns_to_go, timesteps, **kwargs):
        states = states.reshape(1, -1, self.state_dim)
        actions = actions.reshape(1, -1, self.act_dim)
        returns_to_go = returns_to_go.reshape(1, -1, 1)
        timesteps = timesteps.reshape(1, -1)
        ...
        state_mean_preds, state_logvar_preds, _, _ = self.forward(
            states, actions, None, returns_to_go, timesteps, attention_mask=attention_mask, **kwargs)

        return state_mean_preds[0, -1], state_logvar_preds[0, -1]
