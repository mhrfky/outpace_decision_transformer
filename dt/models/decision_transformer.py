...
class DecisionTransformer(TrajectoryModel):
    ...
    def forward(self, states,  returns_to_go, timesteps, attention_mask=None):
        ...
        # Updated code with actions removed
        ...
        # get predictions
        return_preds = self.predict_return(x[:,1])  # Use states to predict next return
    ...
    def get_goal(self, states, returns_to_go, timesteps, **kwargs):
        states = states.reshape(1, -1, self.state_dim)
        returns_to_go = returns_to_go.reshape(1, -1, 1)
        timesteps = timesteps.reshape(1, -1)
        if self.max_length is not None:
        ...
        state_preds, return_preds = self.forward(
            states,  returns_to_go, timesteps, attention_mask=attention_mask, **kwargs)
